...
const BMIPointerRobust = () => {
  ...
  const zoneToastIdRef = useRef(null);
  ...
  const showZoneToast = (zoneName) => {
    if (!zoneName || zoneToastIdRef.current === zoneName) return;
    
    // Dismiss any existing toast
    if (zoneToastIdRef.current) {
      toast.dismiss(zoneToastIdRef.current);
    }
    
    const title = zoneInfo[zoneName]?.title || zoneName;
    const toastId = toast.success(
      <div>
        <p className="font-bold">{title} detected!</p>
        <p className="text-sm">Point here for 1 second to watch the video</p>
      </div>,
      {
        duration: 4000,
    ...
    zoneToastIdRef.current = zoneName;
  };
  ...
  const handleZoneDetection = (zoneName) => {
    if (zoneName !== currentZone) {
      ...
      // Show toast notification when a new zone is detected
      if (zoneName) {
        showZoneToast(zoneName);
      } else if (zoneToastIdRef.current) {
        toast.dismiss(zoneToastIdRef.current);
        zoneToastIdRef.current = null;
      }
      ...
      zoneTimeoutRef.current = setTimeout(() => {
        if (zoneName && zoneInfo[zoneName]) {
          // Navigate to video page instead of showing modal
          navigate('/video-player', { 
            state: { 
              videoUrl: zoneInfo[zoneName].videoUrl,
              title: zoneInfo[zoneName].title,
              zone: zoneName
            }
          });
        }
    ...
    startCamera().then(() => {
      ...
      const detectLoop = async () => {
        ...
        if (!hasBeenAligned.current && matchedIds.length === 4) {
          ...
          toast.success("Poster aligned! Now point with your finger.", {
            duration: 3000,
            position: 'bottom-center'
          });
        }
      ...
      const scaleContainer = () => {
        ...
        const aspectRatio = 1517 / 2200;
        const windowAspect = availableWidth / availableHeight;
        
        let scale;
        if (windowAspect > aspectRatio) {
          // Window is wider than poster - scale based on height
          scale = availableHeight / 2200;
        } else {
          // Window is taller than poster - scale based on width
          scale = availableWidth / 1517;
        }
  ...
  >
    <div
      ...
      style={{
        ...
        border: posterInView ? "none" : "5px dashed red",
      }}
    >
      <video
        ref={videoRef}
      ...
      />
        ...
        <div
          ...
          style={{
            ...
            width: "30px",
            height: "30px",
            background: detectionMode === "hand" ? "rgba(0,255,0,0.5)" : "rgba(255,165,0,0.7)",
            borderRadius: "50%",
...
export default BMIPointerRobust;